...
function ThermostatControl({ device }: ThermostatControlProps) {
  ...
  const haEntityId = device._id || device.haEntityId;
  ...
  const handleDecrease = async () => {
    ...
    try {
      ...
      setError("Error when try to decrease the temperature");
    } finally {
      setLoading(false);
    }
  };
  const handleIncrease = async () => {
    ...
    try {
      ...
      setError("Error when try to increase the temperature");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div>
        <div className="space-y-5">
          <div className="text-center">
            <p className="font-semibold text-gray-500">Living Room</p>
          </div>
